...
class IndexPerformanceTest extends TestBaseSimple
{
    ...
    public function testMetadataOperations(): void
    {
        ...
        // The index already has metadata from constructor
        $metadata = $index->getMetadata();
        $this->assertIsArray($metadata);
        $this->assertArrayHasKey('region', $metadata);
        $this->assertArrayHasKey('asset_class', $metadata);
        $this->assertArrayHasKey('constituents', $metadata);
        $this->assertArrayHasKey('market_cap', $metadata);
        $this->assertArrayHasKey('currency', $metadata);
        
        // Test getting specific metadata values
        $this->assertEquals('US', $index->getMetadataValue('region'));
        $this->assertEquals('equity', $index->getMetadataValue('asset_class'));
        $this->assertEquals(0, $index->getMetadataValue('constituents'));
        $this->assertEquals(0.0, $index->getMetadataValue('market_cap'));
        $this->assertEquals('USD', $index->getMetadataValue('currency'));
        $this->assertNull($index->getMetadataValue('nonexistent'));
        
        // Test with default value
        $this->assertEquals('default', $index->getMetadataValue('nonexistent', 'default'));
    }
