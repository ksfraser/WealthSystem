...
tscoin.c, brownian noise generator, with unfair bias, and cumulative
sum-generates a time series.  The idea is to produce a 1/f squared
power spectrum distribution by running a cumulative sum on white
noise. The program accepts a an unfair bias and a wager factor. See
"Fractals, Chaos, Power Laws," Manfred Schroeder, W. H. Freeman and
Company, New York, New York, 1991, ISBN 0-7167-2136-8, pp 128, pp 151.
...
uniform random deviate between 0.0 and 1.0, with the mean offset
appropriately to provide a Shannon probability, P. For the logistic,
function, the discreet time formula is:
...
$Id: tscoin.c,v 0.0 2006/01/18 20:28:55 john Exp $
$Log: tscoin.c,v $
...
static char rcsid[] = "$Id: tscoin.c,v 0.0 2006/01/18 20:28:55 john Exp $"; /* program version */
...
{
    "\n",
    "Generate an unfair coin toss time series\n",
    "Usage: tscoin [-b n] [-f fraction] [-i value] [-p probability] [-t] [-v]\n"
    "              number\n",
...

{
    ...
    if (argc - optind > 0) /* enough arguments? */
    {
        ...
        for (n = 0; n < number; n ++) /* for each record in the time series */
        {
            ...
            {
                sum = sum + (sum * f) - (b * sum * sum); /* yes, the sum is the sum plus the sum times the wager fraction */
            }

            else
            {
                sum = sum - (sum * f) - (b * sum * sum); /* no, the sum is the sum minus the sum times the wager fraction */
            }
