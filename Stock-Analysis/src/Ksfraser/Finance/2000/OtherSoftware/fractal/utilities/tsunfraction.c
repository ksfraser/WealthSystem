...
tsunfraction.c for making a cumulative sum of the fraction of change
in a time series. The value of a sample in the time series is
multiplied by the running cumulative sum of the time series, and added
to the running sum of the time series. The resultant time series is
printed to stdout. (This program is the inverse of the tsfraction(1)
program.) Note that:

    tsfraction data | tsunfraction

does nothing. An interesting application of this program is:

    tsgaussian -t 10000 | tsmath -t -m 0.01
    | tsmath -t -a 0.0003 | tsunfraction -t

which would manufacture a data file with statistics that are similar to
...
$Id: tsunfraction.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tsunfraction.c,v $
...
static char rcsid[] = "$Id: tsunfraction.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Make a cumulative sum of the fraction of change of a time series\n",
    "Usage: tsunfraction [-i value] [-t] [-v] [filename]\n",
    "    -i value, initial value of output time series\n",
...

{
    ...
    double currentvalue = (double) 0.0, /* value of current sample in time series */
           i = (double) 1.0; /* initial value of output time series */

    FILE *infile = stdin; /* reference to input file */

    while ((c = getopt (argc, argv, "i:tv")) != EOF) /* for each command line switch */
    {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        if (count == 0) /* first record? */
                        {
                            ...
                            if (t == 1) /* print time of samples? */
                            {
                                ...
                                {
                                    (void) printf ("%f\t", atof (token[0]) - (double) 1.0); /* yes, print the first sample's time */
                                }

                                else
                                {
                                    (void) printf ("%d\t", count); /* no, print the first sample's time which is assumed to be the record count */
                                }

                            }

                            (void) printf ("%f\n", (i)); /* print the first sample's value */
                        }

                        i = i + (i * currentvalue); /* the next sample's value is the value of the current sample in the time series multiplied by the cumulative sum, and added to the cumulative sum */
                        ...
                        (void) printf ("%f\n", (i)); /* print the next sample's value */
