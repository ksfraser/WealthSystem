...
foreach my $line (@desc_systems)
{
    ...
    if ( $line =~ s/\\$// ) { # detect and remove \
      $buf = join "", $line, " "; # collect complete line into $line
      next;                   # get next line
    }
    else {
      $line = join "", $buf, $line; # collect complete line into $line
...
#    foreach (@desc_systems)
    foreach my $sys (@systems_list)
    {
	my $sys_manager = $systems->{$sys}{'sys_manager'};
	my $pf_manager = $systems->{$sys}{'pf_manager'};

	# Au boulot
	$analysis = backtest_single($pf_manager, $sys_manager, $broker, 
			$calc, $first, $last, $initial_value);

	# Affichage des résultats
        print "##\n## Analysis of code $code using System Spec\n";
