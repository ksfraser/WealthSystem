...
tsfraction.c for finding the fraction of change in a time series. The
value of a sample in the time series is subtracted from the previous
sample in the time series, and divided by the value of the previous
sample. The fraction time series is printed to stdout.
...
$Id: tsfraction.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tsfraction.c,v $
...
static char rcsid[] = "$Id: tsfraction.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Take the fraction of change of a time series\n",
    "Usage: tsfraction [-t] [-v] [filename]\n",
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        if (count != 0) /* not first record? */
                        {
                            ...
                            (void) printf ("%f\n", ((currentvalue - lastvalue) / lastvalue)); /* print the current sample value minus the last sample value */
                        }
