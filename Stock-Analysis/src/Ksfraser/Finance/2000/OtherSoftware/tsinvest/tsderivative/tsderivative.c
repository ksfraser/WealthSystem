...
tsderivative.c for taking the derivative of a time series.  The value
of a sample in the time series is subtracted from the previous sample
in the time series. The derivative time series is printed to stdout.
...
$Id: tsderivative.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tsderivative.c,v $
...
static char rcsid[] = "$Id: tsderivative.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...

{
    ...
    "Usage: tsderivative [-t] [-v] [filename]\n",
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        if (count != 0) /* not first record? */
                        {
                            ...
                            (void) printf ("%f\n", currentvalue - lastvalue); /* print the current sample value minus the last sample value */
                        }
