...
tslogreturns.c is for taking the logarithmic returns of of a time
series.  The value of a sample in the time series is divided by the
value of the previous sample in the time series, and the logarithm of
the quotient is printed to stdout.

The form of the best fit is e^(at) for exponential least squares fit,
or x^(t) for power least squares fit, or 2^(bt) for binary fit.
...
$Id: tslogreturns.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tslogreturns.c,v $
...
static char rcsid[] = "$Id: tslogreturns.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Take the logarithmic returns of a time series\n",
    "Usage: tslogreturns [-p] [-t] [-v] [filename]\n",
    "    -p, output only the returns formula of the form, e^(at) = x^(t) = 2^(qt)\n",
...

{
    ...
    if (retval == NOERROR) /* enough arguments? */
    {
        ...
        if ((infile = argc <= optind ? stdin : fopen (argv[optind], "r")) != (FILE *) 0) /* yes, open the input file */
        {
            ...
            while (fgets (buffer, BUFLEN, infile) != (char *) 0) /* read the records from the input file */
            {
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        {
                            temp = log (currentvalue / lastvalue); /* save the log of the ratio of the current to last values in the temporary double storage */
                            ...
                            if (p == 0) /* only output formula for least sqares fit? */
                            {
                                ...
                                (void) printf ("%f\n", temp); /* print the log of the ratio of the current to last values */
                            }
            ...
            if (p == 1) /* only output formula for least sqares fit? */
            {
                mean = sumx / ((double) count); /* yes, compute the mean of the numbers in the file */
                /* stddev = (sqrt ((sumsq - sumx * sumx / count) / (count - 1))); */ /* compute the standard deviation of the numbers in the file */
                (void) printf ("e^(%ft) = %f^(t) = 2^(%ft)\n", mean, exp (mean), (mean / log ((double) 2.0))); /* print only output formula for the exponential least sqares fit */
            }
