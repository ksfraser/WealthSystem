...
tslaplacian.c, Laplacian noise generator-generates a time series.  The
idea is to produce a a time series with a Laplacian distribution. The
elements of the time series are made from elements from the uniform
distribution, n, and:

    for n < 0.5, sqrt (0.5) * log (2 * n)
    for n >= 0.5, -sqrt (0.5) * log (2 * (1 - n))

where the sqrt (0.5) is the required scaling for a variance of unity.
...
$Id: tslaplacian.c,v 0.0 2006/01/18 20:28:55 john Exp $
$Log: tslaplacian.c,v $
...
static char rcsid[] = "$Id: tslaplacian.c,v 0.0 2006/01/18 20:28:55 john Exp $"; /* program version */
...

{
    ...
    double uniform, /* an element from the uniform distribution */
           sqrtofonehalf = sqrt ((double) 0.5);
    ...
    if (argc - optind > 0) /* enough arguments? */
    {
        ...
        for (i = 0; i < number; i ++) /* for each record in the time series */
        {
            ...
            uniform = ran1 (&idem); /* get an element from the uniform distribution */

            if (uniform < (double) 0.5) /* element < 0.5? */
            {
                (void) printf ("%f\n", sqrtofonehalf * log (((double) 2.0) * uniform)); /* print the record */
            }

            else /* element >= 0.5 */
            {
                (void) printf ("%f\n", - (sqrtofonehalf * log (((double) 2.0) * (((double) 1.0 - uniform))))); /* print the record */
            }
