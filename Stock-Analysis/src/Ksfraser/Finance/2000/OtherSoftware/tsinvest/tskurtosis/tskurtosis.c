...
tskurtosis.c for finding the coefficient of excess kurtosis of a time
series.  The value of a sample in the time series is analyzed to find
the running coefficient of excess kurtosis to make a new time
...
$Id: tskurtosis.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tskurtosis.c,v $
...
static char rcsid[] = "$Id: tskurtosis.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Find the coefficient of excess kurtosis of a time series\n",
    "Usage: tskurtosis [-p] [-t] [-v] [filename]\n",
    "    -p, don't output the time series, only the coefficient of excess kurtosis\n",
...

{
    ...
    int count = 0, /* input file record counter */
        ...
        p = 0, /* only output root mean square, 0 = no, 1 = yes */
    ...
    if (retval == NOERROR) /* enough arguments? */
    {
        ...
        if ((infile = argc <= optind ? stdin : fopen (argv[optind], "r")) != (FILE *) 0) /* yes, open the input file */
        {
            ...
            while (fgets (buffer, BUFLEN, infile) != (char *) 0) /* read the records from the input file */
            {
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        temp = currentvalue * currentvalue; /* save the square of current value */
                        ...
                        sumforth = sumforth + (temp * temp); /* add the forth power of the value of the current sample in the time series to running value of cumulative sum of forth power */
                        count ++; /* increment the count of records from the input file */

                        if (p == 0) /* only output root mean square flag set? */
                        {
                            temp = sumsquared / (double) count;  /* save the square of the rms */
                            (void) printf ("%f\n", ((sumforth / (double) count) / (temp * temp)) - (double) 3.0); /* no, print the current value of the coefficient of excess kurtosis of the time series, so far */
                        }
            ...
            if (p == 1) /* only output root mean square flag set? */
            {
                temp = sumsquared / (double) count;  /* save the square of the rms */
                (void) printf ("%f\n", ((sumforth / (double) count) / (temp * temp)) - (double) 3.0); /* no, print the current value of the coefficient of excess kurtosis of the time series */
            }
