...
tskurtosiswindow.c for finding the coefficient of excess kurtosis of a
time series.  The value of a sample in the time series is analyzed to
find the running coefficient of excess kurtosis to make a new time
series. The new time series is printed to stdout.

The method used is described in "The CRC Standard Mathematical Tables,
Student Edition, 17th Edition," , Samuel M. Shelby, The Chemical
Rubber Co., 18901 Cranwood Parkway, Cleveland, Ohio 44128 1969,
pp. 563:
...
$Id: tskurtosiswindow.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tskurtosiswindow.c,v $
...
static char rcsid[] = "$Id: tskurtosiswindow.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Find the windowed coefficient of excess kurtosis of a time series\n",
    "Usage: tskurtosiswindow [-t] [-v] [-w size] [filename]\n",
...

{
    ...
    double sumsquared = (double) 0.0, /* running value of cumulative sum of squares */
           sumforth = (double) 0.0, /* running value of cumulative sum of forth power */
           currentvalue, /* value of current sample in time series */
           temp, /* temporary double storage */
           *windowrms, /* reference to the array of the last w many elements for rms from the time series */
           *windowforth; /* reference to the array of the last w many elements for forth power from the time series */
    ...
    if (retval == NOERROR) /* enough arguments? */
    {
        ...
        if ((infile = argc <= optind ? stdin : fopen (argv[optind], "r")) != (FILE *) 0) /* yes, open the input file */
        {
            ...
            if ((windowrms = (double *) malloc ((w) * sizeof (double))) != (double *) 0) /* allocate space for the array of the last w many elements for rms from the time series */
            {

                if ((windowforth = (double *) malloc ((w) * sizeof (double))) != (double *) 0) /* allocate space for the array of the last w many elements for forth power from the time series */
                {
                    ...
                    {
                        windowrms[element] = (double) 0.0; /* initialize each element to zero */
                        windowforth[element] = (double) 0.0; /* initialize each element to zero */
                    }
                        ...
                        if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                        {
                            ...
                            if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                            {
                                ...
                                if (count >= w) /* w many records so far? */
                                {
                                    ...
                                    temp = sumsquared / (double) w;  /* save the square of the rms */
                                    (void) printf ("%f\n", ((sumforth / (double) w) / (temp * temp)) - (double) 3.0); /* yes, print the window value of the coefficient of excess kurtosis of the time series */
                                }

                                temp = currentvalue * currentvalue; /* save the square of the current value */
                                sumsquared = sumsquared - windowrms[element]; /* subtract the value  of the oldest rms sample in the time series from the running value of cumulative sum of squares */
                                ...
                                windowrms[element] = temp; /* replace the oldest value sample of in the time series with the square of the current value from the time series */

                                temp = temp * temp; /* save the forth power of the current value */
                                sumforth = sumforth - windowforth[element]; /* subtract the value  of the oldest forth power sample in the time series from the running value of cumulative sum of forth powers */
                                sumforth = sumforth + temp; /* add the forth power of the value of the current sample in the time series to the running value of cumulative sum of forth powers */
                                windowforth[element] = temp; /* replace the oldest value sample of in the time series with the forth power of the current value from the time series */
