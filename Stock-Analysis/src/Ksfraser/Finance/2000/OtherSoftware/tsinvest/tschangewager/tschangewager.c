...
tschangewager.c, change the unfair returns of a time series. The idea
is to change the returns of a time series which is weighted unfairly,
by changing the increments by a constant factor.  The main function of
this program is regression scenario verification-given an empirical
time series, and a "wager" fraction, speculative market pro forma
performance can be analyzed. The input time series is assumed to be
cumulative sum with fractional or Brownian characteristics.
...
$Id: tschangewager.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tschangewager.c,v $
...
static char rcsid[] = "$Id: tschangewager.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Change the unfair returns of a time series\n",
    "Usage: tschangewager [-d] [-f fraction] [-i value] [-t] [-v] [filename]\n",
    "    -d, the input file is a derivative instead of an integral\n",
    "    -f fraction, incremental changes are multiplied by fraction\n",
...

{
    ...
    while ((c = getopt (argc, argv, "df:i:tv")) != EOF) /* for each command line switch */
    {
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        if (d == 0) /* input file contains differences flag not set? */
                        {
                            ...
                            {
                                temp = (currentvalue - lastvalue) / lastvalue; /* yes, subtract the last value in the time series from the current value, and divide it by the last value in the time series */
                                i = i + (temp * i * f); /* change the wager by a factor of f */
                            }
                        ...
                        {
                            i = i + (currentvalue * i * f); /* change the wager by a factor of f */
