...
tsrootmean.c for finding the root mean of a time series. The number of
consecutive samples of like movements in the time series is tallied,
and the resultant distribution is printed to stdout-a simple random
...
$Id: tsrootmean.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tsrootmean.c,v $
...
static char rcsid[] = "$Id: tsrootmean.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...

{
    ...
    "Usage: tsrootmean [-n n] [-p] [-v] [filename]\n",
        ...
        if ((infile = argc <= optind ? stdin : fopen (argv[optind], "r")) != (FILE *) 0) /* yes, open the input file */
        {
            ...
            if (retval == NOERROR) /* any errors? */
            {
                ...
                if (p == 0) /* only output root mean flag set? */
                {
                    ...
                    {
                        (void) printf ("%d\t%f\n", i, (double) ((double) movements[i] / (double) transitions)); /* print the fraction of the total consecutive movements that were this long */
                    }
                ...
                else
                {
                    ...
                    if (movements_size > 2)
                    {
                        ...
                        (void) printf ("%f\t%f\t%f\t%+ft\n", (double) ((double) movements[1] / (double) movements[0]), running_avg / (double) (j), b, a); /* print the values of the distribution */
                    }
