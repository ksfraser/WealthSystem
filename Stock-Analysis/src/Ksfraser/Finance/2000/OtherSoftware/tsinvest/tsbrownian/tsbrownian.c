...
tsbrownian.c, brownian noise generator-generates a time series.  The
idea is to produce a 1/f squared power spectrum distribution by
running a cumulative sum on white noise. See "Fractals, Chaos, Power
Laws," Manfred Schroeder, W. H. Freeman and Company, New York, New
York, 1991, ISBN 0-7167-2136-8, pp 128.
...
$Id: tsbrownian.c,v 0.0 2006/01/18 20:28:55 john Exp $
$Log: tsbrownian.c,v $
...
static char rcsid[] = "$Id: tsbrownian.c,v 0.0 2006/01/18 20:28:55 john Exp $"; /* program version */
...
{
    "\n",
    "Generate a brownian noise time series\n",
    "Usage: tsbrownian [-t] [-v] number\n",
...

{
    ...
    if (argc - optind > 0) /* enough arguments? */
    {
        ...
        {
            sum = sum + ran1 (&idem) - (double) 0.5; /* compute the cumulative sum to this point in the time series */
