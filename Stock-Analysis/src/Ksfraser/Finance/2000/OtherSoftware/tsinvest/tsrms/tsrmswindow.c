...
tsrmswindow.c for taking the root mean square of a time series.  The
square of a value of a sample in the time series added to the
cumulative sum of the square of the samples to make a new time series
by dividing the cumulative sum of the square of the samples by the
...
$Id: tsrmswindow.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tsrmswindow.c,v $
...
static char rcsid[] = "$Id: tsrmswindow.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Take the windowed root mean square of a time series\n",
    "Usage: tsrmswindow [-t] [-w size] [-v] [filename]\n",
...

{
    ...
    double sumsquared = (double) 0.0, /* running value of cumulative sum of squares */
           ...
           *window; /* reference to the array of the last w many elements from the time series */
    ...
    if (retval == NOERROR) /* enough arguments? */
    {
        ...
        if ((infile = argc <= optind ? stdin : fopen (argv[optind], "r")) != (FILE *) 0) /* yes, open the input file */
        {
            ...
            if ((window = (double *) malloc ((w) * sizeof (double))) != (double *) 0) /* allocate space for the array of the last w many elements from the time series */
            {
                ...
                {
                    window[element] = (double) 0.0; /* initialize each element to zero */
                }
                    ...
                    if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                    {
                        ...
                        if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                        {
                            ...
                            if (count >= w) /* w many records so far? */
                            {
                                ...
                                (void) printf ("%f\n", sqrt (sumsquared / (double) w)); /* yes, print the window value of the root mean square of the time series */
                            }

                            temp = currentvalue * currentvalue; /* save the sqare of the current value */
                            sumsquared = sumsquared - window[element]; /* subtract the value  of the oldest sample in the time series from the running value of cumulative sum of squares */
                            ...
                            window[element] = temp; /* replace the oldest value sample of in the time series with the square of the current value from the time series */
