...
tsgaussian.c, Gaussian noise generator-generates a time series. The
idea is to produce a Gaussian power spectrum distribution.
...
$Id: tsgaussian.c,v 0.0 2006/01/18 20:28:55 john Exp $
$Log: tsgaussian.c,v $
...
static char rcsid[] = "$Id: tsgaussian.c,v 0.0 2006/01/18 20:28:55 john Exp $"; /* program version */
...
{
    "\n",
    "Generate a Gaussian/normally distributed noise time series\n",
    "Usage: tsgaussian [-t] [-v] number\n",
...

{
    ...
    if (argc - optind > 0) /* enough arguments? */
    {
        ...
        for (i = 0; i < number; i ++) /* for each record in the time series */
        {
            ...
            (void) printf ("%f\n", gasdev (&idem)); /* print the record to the time series */
        }
