...
tskalman.c for taking the Kalman filtered average of a time series.
The n'th running Kalman filtered linear average, A, of a time series
is calculated by:

    A  = ((n - 1) / n) A      + (1 / n) a
     n                  n - 1            n

where a is the n'th value in the time series.  The new time series of
the running Kalman filtered average is printed to stdout.
...
$Revision: 0.0 $
$Date: 2006/01/25 19:36:00 $
$Id: tskalman.c,v 0.0 2006/01/25 19:36:00 john Exp $
$Log: tskalman.c,v $
Revision 0.0  2006/01/25 19:36:00  john
Initial version
...
static char rcsid[] = "$Id: tskalman.c,v 0.0 2006/01/25 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Take the Kalman filtered average of a time series\n",
    "Usage: tskalman [-p] [-t] [-v] [filename]\n",
    "    -p, don't output the time series, only the average value\n",
...

{
    ...
    double A = (double) 0.0, /* running value of Kalman filtered average */
    ...
    if (retval == NOERROR) /* enough arguments? */
    {
        ...
        if ((infile = argc <= optind ? stdin : fopen (argv[optind], "r")) != (FILE *) 0) /* yes, open the input file */
        {
            ...
            while (fgets (buffer, BUFLEN, infile) != (char *) 0) /* read the records from the input file */
            {
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        A = ((((double) count - (double) 1) / (double) count) * A) + (((double) 1 / (double) count) * currentvalue); /* include the value of the current sample in the time series to the Kalman filtered average of the time series */

                        if (p == 0) /* only output average flag set? */
                        {
                            (void) printf ("%f\n", A); /* no, print the current value of the Kalman filtered average of the time series, so far */
                        }
            ...
            {
                (void) printf ("%f\n", A); /* no, print the last value of the Kalman filtered average of the time series, so far */
            }
