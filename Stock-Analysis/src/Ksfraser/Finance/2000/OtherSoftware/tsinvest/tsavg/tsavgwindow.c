...
tsavgwindow.c for taking the average of a time series.  The value of a
sample in the time series added to the cumulative sum of the samples
to make a new time series by dividing the cumulative sum by the number
of samples, for each sample. The new time series is printed to stdout.
...
$Id: tsavgwindow.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tsavgwindow.c,v $
...
static char rcsid[] = "$Id: tsavgwindow.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Take the windowed average of a time series\n",
    "Usage: tsavgwindow [-t] [-w size] [-v] [filename]\n",
                    ...
                    if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                    {
                        ...
                        if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                        {
                            ...
                            if (count >= w) /* w many records so far? */
                            {
                                ...
                                (void) printf ("%f\n", sum / (double) w); /* print the current value of the running average of the time series  */
                            }

                            sum = sum - window[element]; /* subtract the value  of the oldest sample in the time series from the cumulative sum of the time series */
                            sum = sum + currentvalue; /* add the value  of the current sample in the time series to the cumulative sum of the time series */
                            window[element] = currentvalue; /* replace the oldest value sample of in the time series with the current value from the time series */
