...
tsintegrate.c for taking the integral of a time series.  The value of
a sample in the time series is added to the previous samples in the
time series. The integral time series is printed to stdout.
...
$Id: tsintegrate.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tsintegrate.c,v $
...
static char rcsid[] = "$Id: tsintegrate.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Take the integral of a time series\n",
    "Usage: tsintegrate [-p] [-P] [-t] [-v] [filename]\n",
    "    -p, print only the final value of the integration\n",
    "    -P, print only the final value of the integration / number of data records\n",
...

{
    ...
    int count = 0, /* input file record counter */
        ...
        p = 0, /* print only the final value of the integration, 0 = no, 1 = yes */
        P = 0, /* print only the final value of the integration / number of records, 0 = no, 1 = yes */
        t = 0, /* print time of samples flag, 0 = no, 1 = yes */
        c; /* command line switch */

    double sum = (double) 0.0; /* running value of samples in time series */

    FILE *infile = stdin; /* reference to input file */

    while ((c = getopt (argc, argv, "pPtv")) != EOF) /* for each command line switch */
    {

        switch (c) /* which switch? */
        {

            case 'p': /* request print only the final value of the integration? */

                p = 1; /* yes, print only the final value of the integration flag */
                break;

            case 'P': /* request print only the final value of the integration / number of records? */

                P = 1; /* yes, print only the final value of the integration / number of records flag */
                break;
    ...
    if (retval == NOERROR) /* enough arguments? */
    {
        ...
        if ((infile = argc <= optind ? stdin : fopen (argv[optind], "r")) != (FILE *) 0) /* yes, open the input file */
        {
            ...
            while (fgets (buffer, BUFLEN, infile) != (char *) 0) /* read the records from the input file */
            {
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        if (t == 1 && p == 0 && P == 0) /* print time of samples, and not printing the final value of the integration? */
                        {
                        ...
                        sum = sum + atof (token[fields - 1]); /* add this sample's value to the running value of samples in time series */

                        if (p != 1 && P != 1) /* print only the final value of the integration or final value of the integration / number of records flag set? */
                        {
                            (void) printf ("%f\n", sum); /* no, print the running value of samples in time series */
                        }
            ...
            if (p == 1) /* print only the final value of the integration set? */
            {
                (void) printf ("%f\n", sum); /* yes, print the running value of samples in time series */
            }

            else if (P == 1) /* print only the final value of the integration / number of records flag set? */
            {
                (void) printf ("%f\n", sum / (double) count); /* yes, print the running value of samples in time series */
            }
