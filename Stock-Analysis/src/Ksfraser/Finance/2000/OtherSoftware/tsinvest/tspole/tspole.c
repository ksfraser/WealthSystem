...
tspole.c for single pole low pass filtering of a time series. The
...
where I is the value of the current sample in the time series, v are the
value of the output time series, and k1 and k2 are constants determined
...
This program is based on "An Analog, Discrete Time, Single Pole Filter,"
John Conover, Fairchild Journal of Semiconductor Progress, July/August,
1978, Volume 6, Number 4, pp. 11.

$Revision: 0.0 $
$Date: 2006/01/18 19:36:00 $
$Id: tspole.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tspole.c,v $
...
static char rcsid[] = "$Id: tspole.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Single pole filter a time series\n",
    "Usage: tspole [-p n] [-t] [-v] [filename]\n",
...

{
    ...
    double currentvalue, /* value of current sample in time series */
           lastvalue = (double) 0.0, /* last value of output time series */
           k1, /* coefficient k1 in the recursive filter */
           k2, /* coefficient k2 in the recursive filter */
           p = (double) 1.0; /* pole increase/decrease factor */

    FILE *infile = stdin; /* reference to input file */

    while ((c = getopt (argc, argv, "p:tv")) != EOF) /* for each command line switch */
    {

        switch (c) /* which switch? */
        {

            case 'p': /* request to change pole increase/decrease factor? */

                p = atof (optarg); /* yes, save the pole increase/decrease factor */
                break;
    ...
    k1 = exp (- ((double) 2.0 * (double) PI * p)); /* calculate the coefficient k1 in the recursive filter */
    k2 = (double) 1.0 - k1; /* calculate the coefficient k2 in the recursive filter */
            ...
            while (fgets (buffer, BUFLEN, infile) != (char *) 0) /* read the records from the input file */
            {
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        {
                            lastvalue = currentvalue; /* yes, initialize the output to the first value in the time series */
                        }

                        else
                        {
                            lastvalue = currentvalue * k2 + lastvalue * k1; /* compute the last value of output time series */
                        }

                        (void) printf ("%f\n", lastvalue); /* print the last value of output time series */
