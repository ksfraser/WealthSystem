...
tscauchy.c, Cauchy distributed noise generator-generates a time
series.  The idea is to produce a 1 / f power spectrum distribution.

The particular method used is from:
...
$Revision: 0.0 $
$Date: 2006/01/01 20:28:55 $
$Id: tscauchy.c,v 0.0 2006/01/01 20:28:55 john Exp $
$Log: tscauchy.c,v $
Revision 0.0  2006/01/01 20:28:55  john
Initial version
...
static char rcsid[] = "$Id: tscauchy.c,v 0.0 2006/01/01 20:28:55 john Exp $"; /* program version */
...
{
    "\n",
    "Generate a Cauchy distributed noise time series\n",
    "Usage: tscauchy [-t] [-v] number\n",
...

{
    ...
    if (argc - optind > 0) /* enough arguments? */
    {
        ...
        for (i = 0; i < number; i ++) /* for each record in the time series */
        {
            ...
            (void) printf ("%f\n", tan ((double) PI * (ran1 (&idem) - (double) 0.5))); /* print the record */
        }
