...
tslognormal.c is for changing the distribution of a time series to a
log-normal distribution. The value of a sample in the time series is
subtracted from the previous sample in the time series, and divided by
the value of the previous sample. This value is multiplied by its
exponentiation, (i.e., e-to-the-power,) and the log-normal fractional
...
$Revision: 0.0 $
$Date: 2006/01/07 19:36:00 $
$Id: tslognormal.c,v 0.0 2006/01/07 19:36:00 john Exp $
$Log: tslognormal.c,v $
Revision 0.0  2006/01/07 19:36:00  john
Initial version
...
static char rcsid[] = "$Id: tslognormal.c,v 0.0 2006/01/07 19:36:00 john Exp $"; /* program version */
...
{
    "\n",
    "Make a log-normal/leptokurtotic distribution of a time series' increments\n",
    "Usage: tslognormal [-t] [-v] [filename]\n",
...

{
    ...
    double currentvalue = (double) 0.0, /* value of current sample in time series */
           ...
           fractional; /* fractional increment, (currentvalue - lastvalue) / lastvalue */
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        if (count != 0) /* not first record? */
                        {
                            ...
                            fractional = ((currentvalue - lastvalue) / lastvalue); /* save the fractional increment, (currentvalue - lastvalue) / lastvalue */

                            if (fractional < (double) 0.0) /* fractional increment, (currentvalue - lastvalue) / lastvalue, negative? */
                            {
                                (void) printf ("%f\n", exp (- fractional) * fractional); /* print the current fractional increment, (currentvalue - lastvalue) / lastvalue, times its exponent */
                            }
