...
tssample.c for sampling a time series.  The value of a sample in the
time series is printed to stdio only if it is a multiple of the
specified interval.
...
$Id: tssample.c,v 0.0 2006/01/18 19:36:00 john Exp $
$Log: tssample.c,v $
...
static char rcsid[] = "$Id: tssample.c,v 0.0 2006/01/18 19:36:00 john Exp $"; /* program version */
...

{
    ...
    "Usage: tssample [-i intervals] [-t] [-v] [filename]\n",
    "    -i, intervals, sample only every i many intervals\n",
...

{
    ...
    int count = 0, /* input file record counter */
        ...
        i = 1, /* sample only i many intervals */
        n = 0, /* interval counter */
    ...
    while ((c = getopt (argc, argv, "i:tv")) != EOF) /* for each command line switch */
    {
        ...
        switch (c) /* which switch? */
        {
            ...
            case 'i': /* request for sample only i many intervals */

                i = atoi (optarg); /* yes, set the sample only i many intervals */
                break;
            ...
            while (fgets (buffer, BUFLEN, infile) != (char *) 0) /* read the records from the input file */
            {
                ...
                if ((fields = strtoken (buffer, parsebuffer, token, token_separators)) != 0) /* parse the record into fields, skip the record if there are no fields */
                {
                    ...
                    if (token[0][0] != '#') /* if the first character of the first field is a '#' character, skip it */
                    {
                        ...
                        if (n == i) /* interval counter equal to sample only i many intervals */
                        {

                            if (t == 1) /* yes, print time of samples? */
                            {
                            ...
                            (void) printf ("%s\n", token[fields - 1]); /* print the current sample value minus the last sample value */
